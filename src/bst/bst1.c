...
void printSpaces(struct bstNode *node, int depth) {
	...
	int leftStreak = 1; /* leftStreak represents a streak of left nodes */
	int rightStreak = 1; /* rightStreak represents a streak of right nodes */
	...
	while (l > 0) {
	/*	if ((node->parent->left == node && leftStreak) ||
		    (node->parent->right == node && rightStreak)) {
			str[l] = ' ';
		} else {
			str[l] = '|';
		}
		...
		if (node->parent->left == node) {
			leftStreak = 1 rightStreak = 0;
		}*/
		/* if this node is a left node */
		if (node->parent->left == node) {
			if (leftStreak) { /* and there has been a streak of left nodes */
				str[l] = ' '; /* print spaces only */
			} else {
				str[l] = '|'; /* otherwise, print '|' and turn on the left streak */
			}
			leftStreak = 1;
			rightStreak = 0; /* breaks the right streak (since the current node is a left node) ;*/

		/* if this node is a right node */
		} else {
			if (rightStreak) { /* and there has been a streak of right nodes */
				str[l] = ' '; /* print spaces only */
			} else {
				str[l] = '|'; /* otherwise, print '|' and turh on the right streak */
			}
			rightStreak = 1;
			leftStreak = 0; /* breaks the left streak (since the current node is a left node) */
		}
		str[l-1] = ' ', str[l-2] = ' '; /* pad two additional spaces per level */
		l -= 3; /* decrement l */
		node = node->parent; /* nove to parent node */
	}
